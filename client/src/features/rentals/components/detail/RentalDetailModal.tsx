...
export function RentalDetailModal({
  ...
  // Переключаем таб, когда модалка открывается с новым rentalId
  useEffect(() => {
    if (rentalId !== null) {
      setTab(initialTab);
    }
  ...
  function transition(status: RentalStatus) {
    if (!rentalId) return;
    updateStatus.mutate({ id: rentalId, status });
  }
          ...
          <div className="border-b border-border p-6">
            <div className="mb-5 flex items-start justify-between gap-4">
              ...
              <div className="flex shrink-0 items-center gap-2">
                {detailQuery.data.status === "pending" && detailQuery.data.hold_expires_at && (
                  <HoldTimer expiresAt={detailQuery.data.hold_expires_at} />
                )}
                    ...
                    <div className="flex items-start gap-3 rounded-xl border border-[hsl(var(--warning))]/30 bg-[hsl(var(--warning))]/10 p-3">
                      ...
                      <div className="min-w-0 flex-1 space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          Подтвердите оплату в блоке «Операции» ниже, чтобы
                          закрыть сделку.
                        </div>
                        <button
                          type="button"
